import 'package:coffee_shop_app/models/news.dart';
...
class HomeProvider with ChangeNotifier {
  ...
  Future<List<String>> getImageSlide() async {
    ...
    Map<String, dynamic>.from(data.snapshot.value).forEach((key, value) {
      listImageSlide.add(value);
    });
    listImageSlideURL.clear();
    for (var i in listImageSlide) {
      storageReference =
          FirebaseStorage.instance.ref().child('news/').child('intro/$i');
  ...
  Future<List<News>> getNews1() async {
    ...
    listNews1.clear();
    var data =
        await firebaseDatabase.reference().child('News').child('1').once();
    Map<dynamic, dynamic>.from(data.snapshot.value).forEach((key, value) {
      News news = News(
        title: value['title'],
        des: value['des'],
        image: value['image'],
    ...
    for (var i in listNews1) {
      storageReference =
          FirebaseStorage.instance.ref().child('news/').child('1/${i.image}');
      String downloadUrl = await storageReference.getDownloadURL();
  ...
  Future<List<News>> getNews2() async {
    ...
    listNews2.clear();
    var data =
        await firebaseDatabase.reference().child('News').child('2').once();
    Map<dynamic, dynamic>.from(data.snapshot.value).forEach((key, value) {
      News news = News(
        title: value['title'],
        des: value['des'],
    ...
    for (var i in listNews2) {
      storageReference =
          FirebaseStorage.instance.ref().child('news/').child('2/${i.image}');
      String downloadUrl = await storageReference.getDownloadURL();
  ...
  Future<List<News>> getNews3() async {
    ...
    listNews3.clear();
    var data =
        await firebaseDatabase.reference().child('News').child('3').once();
    Map<dynamic, dynamic>.from(data.snapshot.value).forEach((key, value) {
      News news = News(
        title: value['title'],
        des: value['des'],
    ...
    for (var i in listNews3) {
      storageReference =
          FirebaseStorage.instance.ref().child('news/').child('3/${i.image}');
      String downloadUrl = await storageReference.getDownloadURL();
